...
# All the valid tile codes including heads + terminals
VALID_TILE_INTS: builtins.frozenset[int] = frozenset(
...
class FlowFree:
    ...
    def yield_valid_placements(self) -> Iterator[tuple[Coordinate, int]]:
        """Yield all valid placements as (coordinate, color) pairs."""
        # For every non-solved color with no head, yield all adjacent terminal coordinates
        for color, (term1, term2) in self._terminals.items():
            if self._heads.get(color, None) is None and not self.is_color_solved(color):
                for n in term1.neighbors4() + term2.neighbors4():
                    if self.is_legal_move(n, color):
                        yield n, color

        # For every color with a head, yield all adjacent empty coordinates
        for color, head_pos in self._heads.items():
            if head_pos is not None:
                for n in head_pos.neighbors4():
                    if self.is_legal_move(n, color):
                        yield n, color
    ...
    def is_color_solved(self, color: int) -> bool:
        ...
        return self._heads.get(color, None) is None and self._connected(t1, t2, color)
    ...
    def display(self) -> None:
        ...
        # This palette was generated by ChatGPT so I cannot verify the correctness of int -> color mapping lol
        palette = [
            196,  # bright red
            46,  # bright green
            21,  # bright blue
